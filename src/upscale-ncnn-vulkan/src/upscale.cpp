...
static const uint32_t upscale_preproc_spv_data[] = {
#include "upscale_preproc.spv.hex.h"
};
static const uint32_t upscale_preproc_fp16s_spv_data[] = {
#include "upscale_preproc_fp16s.spv.hex.h"
};
static const uint32_t upscale_preproc_int8s_spv_data[] = {
#include "upscale_preproc_int8s.spv.hex.h"
};
static const uint32_t upscale_postproc_spv_data[] = {
#include "upscale_postproc.spv.hex.h"
};
static const uint32_t upscale_postproc_fp16s_spv_data[] = {
#include "upscale_postproc_fp16s.spv.hex.h"
};
static const uint32_t upscale_postproc_int8s_spv_data[] = {
#include "upscale_postproc_int8s.spv.hex.h"
};

static const uint32_t upscale_preproc_tta_spv_data[] = {
#include "upscale_preproc_tta.spv.hex.h"
};
static const uint32_t upscale_preproc_tta_fp16s_spv_data[] = {
#include "upscale_preproc_tta_fp16s.spv.hex.h"
};
static const uint32_t upscale_preproc_tta_int8s_spv_data[] = {
#include "upscale_preproc_tta_int8s.spv.hex.h"
};
static const uint32_t upscale_postproc_tta_spv_data[] = {
#include "upscale_postproc_tta.spv.hex.h"
};
static const uint32_t upscale_postproc_tta_fp16s_spv_data[] = {
#include "upscale_postproc_tta_fp16s.spv.hex.h"
};
static const uint32_t upscale_postproc_tta_int8s_spv_data[] = {
...
UPSCALE::~UPSCALE() {
    ...
    {
        delete upscale_preproc;
        delete upscale_postproc;
    }
...
#if _WIN32
int UPSCALE::load(const std::wstring& parampath, const std::wstring& modelpath)
#else

int UPSCALE::load(const std::string &parampath, const std::string &modelpath)
#endif
{
        ...
        // initialize preprocess and postprocess pipeline
    {
        ...
        upscale_preproc = new ncnn::Pipeline(vkdev);
        upscale_preproc->set_optimal_local_size_xyz(32, 32, 3);

        upscale_postproc = new ncnn::Pipeline(vkdev);
        upscale_postproc->set_optimal_local_size_xyz(32, 32, 3);

        if (tta_mode) {
            if (net.opt.use_fp16_storage && net.opt.use_int8_storage)
                upscale_preproc->create(upscale_preproc_tta_int8s_spv_data,
                                           sizeof(upscale_preproc_tta_int8s_spv_data), specializations);
            else if (net.opt.use_fp16_storage)
                upscale_preproc->create(upscale_preproc_tta_fp16s_spv_data,
                                           sizeof(upscale_preproc_tta_fp16s_spv_data), specializations);
            else
                upscale_preproc->create(upscale_preproc_tta_spv_data, sizeof(upscale_preproc_tta_spv_data),
                                           specializations);

            if (net.opt.use_fp16_storage && net.opt.use_int8_storage)
                upscale_postproc->create(upscale_postproc_tta_int8s_spv_data,
                                            sizeof(upscale_postproc_tta_int8s_spv_data), specializations);
            else if (net.opt.use_fp16_storage)
                upscale_postproc->create(upscale_postproc_tta_fp16s_spv_data,
                                            sizeof(upscale_postproc_tta_fp16s_spv_data), specializations);
            else
                upscale_postproc->create(upscale_postproc_tta_spv_data, sizeof(upscale_postproc_tta_spv_data),
                                            specializations);
        } else {
            if (net.opt.use_fp16_storage && net.opt.use_int8_storage)
                upscale_preproc->create(upscale_preproc_int8s_spv_data, sizeof(upscale_preproc_int8s_spv_data),
                                           specializations);
            else if (net.opt.use_fp16_storage)
                upscale_preproc->create(upscale_preproc_fp16s_spv_data, sizeof(upscale_preproc_fp16s_spv_data),
                                           specializations);
            else
                upscale_preproc->create(upscale_preproc_spv_data, sizeof(upscale_preproc_spv_data),
                                           specializations);

            if (net.opt.use_fp16_storage && net.opt.use_int8_storage)
                upscale_postproc->create(upscale_postproc_int8s_spv_data,
                                            sizeof(upscale_postproc_int8s_spv_data), specializations);
            else if (net.opt.use_fp16_storage)
                upscale_postproc->create(upscale_postproc_fp16s_spv_data,
                                            sizeof(upscale_postproc_fp16s_spv_data), specializations);
            else
                upscale_postproc->create(upscale_postproc_spv_data, sizeof(upscale_postproc_spv_data),
                                            specializations);
        }
    }
...
int UPSCALE::process(const ncnn::Mat &inimage, ncnn::Mat &outimage) const {
    if (!vkdev) {
        // cpu only
        ...
        for (int xi = 0; xi < xtiles; xi++) {
            ...
            if (tta_mode) {
                ...
                ncnn::VkMat in_alpha_tile_gpu;
                {
                    ...
                    cmd.record_pipeline(upscale_preproc, bindings, constants, dispatcher);
                }
                ...
                // postproc
                {
                    ...
                    cmd.record_pipeline(upscale_postproc, bindings, constants, dispatcher);
                }
            } else {
                // preproc
...
int UPSCALE::process_cpu(const ncnn::Mat &inimage, ncnn::Mat &outimage) const {
    const unsigned char *pixeldata = (const unsigned char *) inimage.data;
    const int w = inimage.w;
    const int h = inimage.h;
    const int channels = inimage.elempack;
